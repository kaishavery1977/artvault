...
void main() {
  ...
  group('Passcode hash (PBKDF2-HMAC-SHA256)', () {
    ...
    test('matches the RFC 6070 vector for c=2', () async {
      ...
      expect(await AuthRepository.instance.verifyPasscode('password'), isTrue);
      expect(await AuthRepository.instance.verifyPasscode('password2'),
          isFalse);
    });

    test('setPasscode stores a salted v2 digest and verifies round-trip',
        () async {
    ...
    test('legacy v1 salted-SHA256 digests still verify and auto-upgrade',
        () async {
  ...
  group('Passcode brute-force throttle', () {
    ...
    test('escalates exponentially after the threshold, capped at 10 minutes',
        () async {
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          Duration.zero); // 1
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          Duration.zero); // 2
      expect(await AuthRepository.instance.registerPasscodeFailure(),
      ...
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          Duration.zero); // 4
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(seconds: 30)); // 5 → first lockout
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(seconds: 60)); // 6
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(seconds: 120)); // 7
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(seconds: 240)); // 8
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(seconds: 480)); // 9
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(minutes: 10)); // 10 → capped
      expect(await AuthRepository.instance.registerPasscodeFailure(),
          const Duration(minutes: 10)); // 11 → stays capped
    });

    test('a live lockout is visible and survives (persisted timestamp)',
        () async {
      for (var i = 0; i < 5; i++) {
        await AuthRepository.instance.registerPasscodeFailure();
      }
    ...
    test('resetPasscodeAttempts clears the failure count and lockout',
        () async {
      for (var i = 0; i < 7; i++) {
        await AuthRepository.instance.registerPasscodeFailure();
      }
      await AuthRepository.instance.resetPasscodeAttempts();
      expect(await AuthRepository.instance.passcodeLockRemaining(),
          Duration.zero);
      expect(
          int.tryParse(_secureStore[AppConstants.kPasscodeFailures] ?? '') ?? 0,
          0);
...
String cryptoSha256(String input) =>
    _sha256(utf8.encode(input)).map((b) => b.toRadixString(16).padLeft(2, '0'))
        .join();
